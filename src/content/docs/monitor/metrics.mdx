...
description: Complete reference for all Prometheus metrics exported by Onax.
section: monitor
order: 2
---
...
Onax exports Prometheus metrics for every CronJob in your cluster. All metrics are labeled with `namespace` and `cronjob` for easy filtering and aggregation.
...
Onax complements kube-state-metrics — it doesn't replace it. kube-state-metrics provides general Kubernetes object state (pod counts, deployment replicas, etc.). Onax adds CronJob-specific execution tracking that kube-state-metrics doesn't cover: execution duration, missed schedules, and per-run success/failure tracking.
